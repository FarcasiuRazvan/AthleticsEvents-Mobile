...
class DatabaseHelper {
  ...
  Future<Database> initializeDatabase() async {
    ...
    String path = directory.path + 'athletics_events3.db';
  ...
  void _createDb(Database db, int newVersion) async {
    ...
    await db.execute('CREATE TABLE $toPostTable($colId INTEGER PRIMARY KEY AUTOINCREMENT, $colTitle TEXT, '
        '$colPhoto TEXT, $colDescription TEXT, $colServerId TEXT)');
  ...
  Future<int> updateEvent(AthleticEvent ev) async {
    ...
    print("NEBUNIE"+ev.photo+" "+ev.description+" "+ev.title+" "+ev.serverId);
    var result = await db.rawUpdate('UPDATE events_table SET url_photo = ?, description = ?, serverId = ? WHERE title = ?',[ev.photo,ev.description,ev.serverId,ev.title]);
    return result;
  }
  ...
  Future<List<AthleticEvent>> getEventsList() async {
    ...
    for (int i = 0; i < count; i++) {
      ...
      noteList.add(AthleticEvent(title:titlu,url_photo: url_photo,description: description,serverId: serverId));
    }
